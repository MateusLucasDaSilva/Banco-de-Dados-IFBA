...
select idfuncionario, count(idos) from os group by idfuncionario having idFuncionario >4;

-- (Busca)departamentos que realizaram mais de um servico
SELECT D.nomeDepartamento, count(*) As quantidade from Departamento D join OS O on (D.idDepartamento = O.idDepartamento) group by D.nomeDepartamento having Quantidade >= 2;

SELECT D.nomeDepartamento, (SELECT count(O.idDepartamento) FROM OS O WHERE O.idDepartamento = D.idDepartamento )As quantidade from DEPARTAMENTO D group by D.idDepartamento having quantidade >0;
